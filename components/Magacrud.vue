...
const dataGridColumns_table = ref([]);
...
onMounted(async () => {
  ...
  dataGridColumns_table.value = Object.entries(props.schema.schema)
  .map(([key, val]) => ({
    id: key,
    label: val.label,
    type: val.type,
    null: val.null,
    hide: val.hide || false,
    notshow: val.notshow || false,
    options: val.options,
    placeholder: val.placeholder,
  }))
  .filter(col => !col.notshow) // já remove do array
...
const deleteSelectedDataItems = () => {
  data.value = data.value.filter(
    (val) => !selectedDataItems.value.includes(val)
  );
...
async function handleImageUpload(x) {
  try {
    ...
    Array.from(files.value).map(async (file, index) => {
      ...
      const { data, pending, error, refresh } = await useAsyncData("data", () =>
      ...
      <div class="card">
        ...
        >
                ...
                <div>
                  <div class="mt-3">
                    ...
                    <Button
                      ...
                      :disabled="
                        !selectedDataItems || !selectedDataItems.length
                      "
                    />
                  </div>
                </div>
          ...
          <Column
            ...
            v-for="item in dataGridColumns_table"
          ...
          >
            <template #body="slotProps">
              <div v-if="item.notshow !== true">
                <div v-if="item.type.toLowerCase() == 'upload'">
                  <a :href="'/upload/' + slotProps.data[item.id]">{{
                ...
                <div v-if="['select'].includes(item.type.toLowerCase())">
                  {{
                    item.options.find((x) => x.code == slotProps.data[item.id])
                      .name
                  }}
                </div>
                <div v-if="['multiselect'].includes(item.type.toLowerCase())">
                  {{
                    slotProps.data[item.id]
                      .map((t) =>
                        item.options.find((x) => x.code == t)
                          ? item.options.find((x) => x.code == t).name
                          : "-"
                      )
                      .join(", ")
                  }}
          ...
          <Column headerStyle="min-width:10rem;">
            <template #body="slotProps">
              <Button
                ...
                @click="editDataItem(slotProps.data)"
              />
              <Button
                icon="pi pi-trash"
                class="p-button-rounded p-button-warning mt-2"
                @click="confirmDeleteDataItem(slotProps.data)"
              />
            </template>
          </Column>
        </DataTable>
        ...
        <Dialog
          v-model:visible="dataItemDialog"
          :style="{ width: '450px' }"
          :header="dataTitle"
          :modal="true"
          class="p-fluid"
        >
          <div v-for="item in dataGridColumns" :key="item.id" class="field">
            <div v-show="item.id.toLowerCase() !== 'id'">
              ...
              <MultiSelect
                v-if="item.type.toLowerCase() == 'multiselect'"
                display="chip"
                v-model="dataItem[item.id]"
                :options="item.options"
                optionValue="code"
            ...
            <form
              ref="formUploadFile"
              v-if="item.type.toLowerCase() == 'upload'"
              _style="display: none;"
              @submit.prevent="handleImageUpload(item.id)"
            >
            ...
            <small class="p-invalid" v-if="submitted && !dataItem.name"
              >Preencha {{ item[1] }}</small
            >
          </div>
        ...
        <Dialog
          v-model:visible="deleteDataItemDialog"
          :style="{ width: '450px' }"
          header="Confirm"
          :modal="true"
        >
          ...
          <template #footer>
            <Button
              ...
              icon="pi pi-times"
              class="p-button-text"
              @click="deleteDataItemDialog = false"
            />
            <Button
              label="Sim"
              icon="pi pi-check"
        ...
        <Dialog
          v-model:visible="deleteDataItemsDialog"
          :style="{ width: '450px' }"
          header="Confirm"
          :modal="true"
        >
          <div class="flex align-items-center justify-content-center">
            ...
            />
            <span v-if="dataItem"
              >Confirma exclusão de itens selecionados?</span
            >
          </div>
          <template #footer>
            <Button
              ...
              icon="pi pi-times"
              class="p-button-text"
              @click="deleteDataItemsDialog = false"
            />
            <Button
              label="Sim"
              icon="pi pi-check"
              class="p-button-text"
              @click="deleteSelectedDataItems"
            />
          </template>
        </Dialog>
      </div>
